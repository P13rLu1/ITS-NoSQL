...
db.movies.aggregate({$unwind : "$cast"},{$group : {_id : "$cast", count : {$sum : 1}}},{$match : {count : {$gt : 50}}},{$sort : {count : -1}})
...
// View
db.createView("imdbgt8", "movies", [{$match : {"imdb.rating" : {$gt : 8}}}]);

// explain
db.movies.find({"year": {$gt: 2000}}).sort({"imdb.rating": -1}).explain("executionStats");
//index
